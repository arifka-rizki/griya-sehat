<?php
  ...
  if($_SESSION["checkdate"]===true){
      $curdate = $_SESSION["datex"];
  }
  else{
      $curdate = date('Y-m-d');
  }
  ...
  $sql = "SELECT * from reservasi where Tanggal='$curdate'";
    ...
    <div id="wrapper">
        ...
        <div class="card mb-3">
          <div class="card-header">
            ...
            Daftar Reservasi</div>
          <div class="card-body">
            <div class="table-responsive">
            <form method="POST" action="changedate.php">
                Tanggal
            <input type="date" id="TanggalP" name="tanggal" value=<?php echo $curdate ?> required>
            <button type="submit">Submit</button>
            </form>
            <?php if(count($dataresv)>1) : ?>
            <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                <thead>
                  <tr>
                    <th><?php echo implode('</th><th>', array_keys(current($dataresv))); ?></th>
                  </tr>
                </thead>
                <tbody>
                  <?php foreach ($dataresv as $rows): array_map('htmlentities', $rows); ?>
                      <tr>
